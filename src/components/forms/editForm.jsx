...
class AddForm extends Component {
    ...
    constructor(props) {
        ...
        this.state = {
                id: appointment.id,
                start: {data:`${appointment.start.hour}:${appointment.start.minute}`, valid: true, regex: /^([0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/, class:1},
                end: {data:`${appointment.end.hour}:${appointment.end.minute}`, valid: true, regex: /^([0-9]|1[0-9]|2[0-3]):[0-5][0-9]$/, class:1},
                name: {data: appointment.name, valid: true, regex: /^[a-zA-Z0-9]{1,30}$/, class:1},
                description: {data: appointment.description, valid: true, regex: /^[ a-zA-Z0-9]{1,400}$/, class:1},
                priority: {data: appointment.priority, valid: true, regex: /^(low|medium|high)$/, class:1},
                formValid: true,
                backendErrors: '',
                inputClass: {
                    1: 'formInput',
    ...
    submitForm(e) {
        ...
        if (this.state.formValid) {
            ...
            try { // should be promise later when you do async in action creator
                this.props.dispatch(actionCreators.editAppointmentActionCreator(appointment.selectedDate, appointment.data));
    ...
    render() {
        ...
        return (
                <form className="appointmentForm">
                    ...
                    <div className="input-wrapper">
                        <button onClick={this.submitForm} disabled={!this.state.formValid} className={this.state.formValid ? 'formInput valid' : 'formInput invalid'} type="submit">EDIT APPOINTMENT</button>
                    </div>
